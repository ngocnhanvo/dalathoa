...
var format_nhacungcap = {
    url: 'Controller/JqGrid/JQGridMD_00_DTKD_NCCLoad.ashx?ma_module=MD_00_DTKD_NCC&ma_menu=MN_01_DTKD&id=null&id_sel=&module_select=1',
    colModel: [
        { key: true, fixed: true, label: 'Mã NCC', name: 'ma_dtkd', index: ' dtkd.ma_dtkd ', width: 80, editable: true, editrules: { edithidden: true }, hidden: false, align: 'left', searchoptions: { sopt: ['bw'] }, editoptions: { maxLength: 255 }, frozen: true, formoptions: { label: 'Mã đối tác kinh doanh' } },
        { key: false, fixed: true, label: 'Tên NCC', name: 'ten_dtkd', index: ' dtkd.ten_dtkd ', width: 160, editable: true, editrules: { edithidden: true }, hidden: false, align: 'left', searchoptions: { sopt: ['bw'] }, edittype: 'textarea', frozen: false, formoptions: { label: 'Tên đối tác' } },
    ...
    select: function ($elem, cellSav) {
        ...
        if ($elem.prop('disabled') != true) {
            $elem.val(cellSav.ma_dtkd);
            $elemFrm.find('#ten_dtkd').val(cellSav.ten_dtkd);
            $elemFrm.find('#diachi').val(cellSav.diachi);
            $elemFrm.find('#diachigiaohang').val(cellSav.diachi);
            $elemFrm.find('#nguoilienhe').val(cellSav.daidien);
            ...
            //kiotv
            $elemFrm.find('#ten_dtkd_kov').val(cellSav.ten_dtkd);
            $elemFrm.find('#tel_kov').val(cellSav.tel);
            $elemFrm.find('#diachi_kov').val(cellSav.diachi);
        }
    },
    create: function (elem) {
        ...
        $(elem).combogrid({
            ...
            munit: 'px',
            replaceNull: false,
            url: format_nhacungcap.url,
            colModel: format_nhacungcap.colModel,
            postData: {
                _search: true
            },
            appendData: function () {
            ...
            select: function (event, ui) {
                format_nhacungcap.select($(elem), ui.item);
                return false;
            }
        });

        $(elem).addClass('format_vnn');
        $(elem).parent().append(`<span onclick="format_nhacungcap.search(this)" class="format_dtkd formatsearch" />`);
    },
            ...
            open: function (event, ui) {
                $gridMaster.jqGrid({
                    ...
                    loadComplete: function (data) {
                        ...
                        const $timkiem = $(`.gs_ma_dtkd.gs_${gridMasterId}`);
                        if (load_sp == 0) {
            ...
            buttons: [
                {
                    ...
                    text: 'Áp dụng',
                    click: function () {
                        format_nhacungcap.select($elem, cellSav);
                        $(this).dialog('destroy').remove();
                        $elem.focus();
                    }
                },
