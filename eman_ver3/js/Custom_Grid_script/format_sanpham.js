...
var format_sanpham = {
    ...
    select: function ($elem, cellSav) {
        ...
        if ($elem.prop('disabled') != true) {
            $elem.val(cellSav.ma_sanpham);
            $elemFrm.find('#mota_tiengviet').val(cellSav.mota_tiengviet);
            // Tìm option có text trùng khớp hoàn toàn
            var $option = $('#md_donvitinhsanpham_id option').filter(function () {
                return $(this).text() === cellSav.md_donvitinhsanpham_id;
            });
            if ($option.length > 0) {
                // Gán giá trị và kích hoạt sự kiện change nếu cần
                $('#md_donvitinhsanpham_id').val($option.val()).change();
            }
        }
    },
    create: function (elem) {
        ...
        $(elem).combogrid({
            ...
            munit: 'px',
            replaceNull: false,
            url: format_sanpham.url,
            colModel: format_sanpham.colModel,
            postData: {
                _search: true
            },
            appendData: function () {
            ...
            select: function (event, ui) {
                format_sanpham.select($(elem), ui.item);
                return false;
            }
        });

        $(elem).addClass('format_vnn');
        $(elem).parent().append(`<span onclick="format_sanpham.search(this)" class="format_dtkd formatsearch" />`);
        setTimeout(function () { $('#tr_sanpham_id').hide(); $('#tr_md_sanpham_id').hide(); }, 10);
    },
            ...
            buttons: [
                {
                    ...
                    text: 'OK',
                    click: function () {
                        format_sanpham.select($elem, cellSav);
                        $(this).dialog('destroy').remove();
                        $elem.focus();
                    }
                },
