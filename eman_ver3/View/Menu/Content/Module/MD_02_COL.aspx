...
<%=VNN_JQGridver2.get_layout_face() %>
    ...
    jQuery('#' + tengrid2).jqGrid('navGrid', '#pager' + tengrid2,
        ...
        beforeSubmit: function (postdata, formid) {
            ...
            postdata.label = encodeHTML(postdata.label);
    ...
    function action_grid2(action, column) {
        if (column == 'edittype') {
            $.get('Controller/JQGridModify/JQGrid<%=ma_module %>Modify.ashx?oper=sel_edittype&id=' + $('#' + column).val(), function (value_select) {
                if (value_select != null) {
                    value_select = value_select.split('(##)');
                    $('#ma_edittype').val(value_select[0]);
                    $('#editoptions').val(value_select[1]);
                    $('#formatoptions').val(value_select[2]);
                    $('#formatter').val(value_select[3]);
                }
            });
        }
        if (action == "add") {
            ...
            if (module_thuake == 1) {
                khoa_field(tengrid2);
                mokhoa_column('hidden');
                mokhoa_column('editable');
            }
        }
        $('#tr_ma_edittype').hide();
    }
