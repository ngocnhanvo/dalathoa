...
using DataAcess;
using System.Linq;
public partial class PrintControllers_MD_00_Nhapnoibo_lenhNhapKho : System.Web.UI.Page
{
    ...
    public String CreateSql(HttpContext context)
    {
        ...
        string loaiCT0 = Helper.arrLoaiCT_LNK[0];
        string loaiCT1 = Helper.arrLoaiCT_LNK[3];
        string sql = $@"
		    declare @id nvarchar(32)= '{id}'
            select
                (case when sp.vattu = 1 then upper(N'{loaiCT1}') else upper(N'{loaiCT0}') end) as loaiCT,
	            vcnb.sochungtu as sophieu,
	            format(vcnb.ngaychuyen, 'dd/MM/yyyy') as ngaylap,
	            kho.ten_kho as kho,
	            replace(vcnb.donhang_thamchieu, ',', CHAR(13)+CHAR(10)) as donhang,
                ...
                null as ghichu,
                null as soDMH
            from md_nhapkhonb vcnb
                left join md_kho kho on kho.md_kho_id = vcnb.denkho
                left join md_nhapkhonb_cdh cdvc on cdvc.md_nhapkhonb_id = vcnb.md_nhapkhonb_id
            ...
            where
                1=1
                and vcnb.trangthai != '{Helper.SOANTHAO}'
                and vcnb.md_nhapkhonb_id = @id
                and cdvc.sl_nhap > 0
            order by
	            sp.ma_sanpham
		";
