...
public partial class PrintControllers_MD_00_Baocaoketoan_BaoCaoXuatNhapTonTheoPO : System.Web.UI.Page
{
        ...
        string sql = $@"
            ...
            from (
                select
	                ...
	                A.tenVTHH,
	                A.dvt,
	                sum(isnull(A.sldk, 0)) as tonDK,
	                sum(isnull(A.slntk, 0)) as nhapTK,
	                sum(isnull(A.slxtk, 0)) as xuatTK,
                    @tenkho as kho,
                    A.po
                from (
	                select
                ...
                group by
	                A.maVTHH, A.tenVTHH, A.dvt, A.po
            )A
            where 
                A.tonDK != 0 or A.nhapTK != 0 or A.xuatTK != 0
            order by
	             A.po, A.maVTHH
		";
