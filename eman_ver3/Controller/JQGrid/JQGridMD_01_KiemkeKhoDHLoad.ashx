<%@ WebHandler Language="C#" Class="JQGridMD_01_KiemkeKhoDHLoad" %>
using System;
using System.Web;
using System.Linq;
using DataAcess;
public class JQGridMD_01_KiemkeKhoDHLoad : IHttpHandler , System.Web.SessionState.IRequiresSessionState {
...
string id_vaitro = Security.id_vaitro(context);            string where_vaitro = db.ad_role_where.Where(s => s.ad_role_id == id_vaitro & s.ad_module_id == mod_.ad_module_id).Select(s => s.where_sql).FirstOrDefault();
            ...
            sELeCt " + select_sql + ROW_NUMBER + " as RowNum" +
            @" fRoM {1}
