<%@ WebHandler Language="C#" Class="JQGridMD_01_LichSuNXKhoModify" %>
using System;
using System.Web;
using System.Linq;
using System.Data.Linq;
using DataAcess;
public class JQGridMD_01_LichSuNXKhoModify : IHttpHandler, System.Web.SessionState.IRequiresSessionState
{
    ...
    public void add(HttpContext context)
    {
        ...
        try
        {
            ...
            if (msg.Length <= 0)
            {
                string action = "add";
                string[] column_ex = { };
                string ten_table = "md_kho_giaodich";
    ...
    public void edit(HttpContext context)
    {
        ...
        {
            string id = context.Request.Form["id"];
            string object_ = db.md_kho_giaodich.Where(p => p.md_kho_giaodich_id == id).Select(s => s.md_kho_giaodich_id).Take(1).FirstOrDefault();
            if (object_ == null)
            {
...
string object_ = db.md_kho_giaodich.Where(p => p.md_kho_giaodich_id == id_del_).Select(s => s.md_kho_giaodich_id).Take(1).FirstOrDefault();
                if (object_ == null)
                {
