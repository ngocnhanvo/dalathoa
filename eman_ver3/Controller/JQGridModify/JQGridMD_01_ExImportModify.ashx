<%@ WebHandler Language="C#" Class="JQGridMD_01_ExImportModify" %>
using System;
using System.Web;
using System.Linq;
using System.Data.Linq;
using DataAcess;
public class JQGridMD_01_ExImportModify : IHttpHandler, System.Web.SessionState.IRequiresSessionState
{
    ...
    public void add(HttpContext context)
    {
        ...
        try
        {
			...
			if(context.Request.Form["row_ex"] == "0")
			{
				msg = "false#Dòng ngoại lệ phải khác 0";
			}
			
            if (msg.Length <= 0)
            {
                string action = "add";
                string[] column_ex = { "ad_import_id" };
                string ten_table = "ad_import_ex";
				VNN_Function.SetFormValue("ad_import_id", id);
    ...
    public void edit(HttpContext context)
    {
        ...
        {
            string id = context.Request.Form["id"];
            string object_ = db.ad_import_ex.Where(p => p.ad_import_ex_id == id).Select(s => s.ad_import_ex_id).Take(1).FirstOrDefault();
            if (object_ == null)
            {
...
string object_ = db.ad_import_ex.Where(p => p.ad_import_ex_id == id_del_).Select(s => s.ad_import_ex_id).Take(1).FirstOrDefault();
                if (object_ == null)
                {
