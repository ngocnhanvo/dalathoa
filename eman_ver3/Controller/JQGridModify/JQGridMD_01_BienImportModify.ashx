<%@ WebHandler Language="C#" Class="JQGridMD_01_BienImportModify" %>
using System;
using System.Web;
using System.Linq;
using System.Data.Linq;
using DataAcess;
public class JQGridMD_01_BienImportModify : IHttpHandler, System.Web.SessionState.IRequiresSessionState
{
    ...
    public void add(HttpContext context)
    {
        ...
        try
        {
            ...
            if (msg.Length <= 0)
            {
                ...
                string ten_table = "ad_import_ava";
                VNN_Function.SetFormValue("ad_import_id", id_parent );
    ...
    public void edit(HttpContext context)
    {
        ...
        {
            string id = context.Request.Form["id"];
            string object_ = db.ad_import_ava.Where(p => p.ad_import_ava_id == id).Select(s => s.ad_import_ava_id).Take(1).FirstOrDefault();
            if (object_ == null)
            {
...
string object_ = db.ad_import_ava.Where(p => p.ad_import_ava_id == id_del_).Select(s => s.ad_import_ava_id).Take(1).FirstOrDefault();
                if (object_ == null)
                {
