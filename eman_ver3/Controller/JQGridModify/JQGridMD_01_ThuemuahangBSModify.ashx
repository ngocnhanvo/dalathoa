<%@ WebHandler Language="C#" Class="JQGridMD_01_ThuemuahangModify" %>
using System;
using System.Web;
using System.Linq;
using System.Data.Linq;
using DataAcess;
public class JQGridMD_01_ThuemuahangModify : IHttpHandler, System.Web.SessionState.IRequiresSessionState
{
    ...
    public void add(HttpContext context)
    {
        ...
        try
        {
            ...
            if (msg.Length <= 0)
            {
                string action = "add";
                string[] column_ex = { };
                string ten_table = "c_donmuahang_thue";
    ...
    public void edit(HttpContext context)
    {
        ...
        {
            string id = context.Request.Form["id"];
            string object_ = db.c_donmuahang_thue.Where(p => p.c_donmuahang_thue_id == id).Select(s => s.c_donmuahang_thue_id).Take(1).FirstOrDefault();
            if (object_ == null)
            {
...
string object_ = db.c_donmuahang_thue.Where(p => p.c_donmuahang_thue_id == id_del_).Select(s => s.c_donmuahang_thue_id).Take(1).FirstOrDefault();
                if (object_ == null)
                {
