...
<head>
    <meta charset="UTF-8">
    <title>Thông báo từ quản trị viên</title>
...
<body>
    ...
    <div class="">
        <div class="">
            <div class="main">
                <h2 style="text-align: center; margin-bottom: 15px; position: relative">
                    <img class="reportPublic left" src="report_image_speakers.png" />
                    BẢNG THÔNG BÁO
                    <img class="reportPublic right" src="report_image_speakers.png" />
                </h2>
                <!-- Another variation with a button -->
                
		        <div class="input-group">
                    <input type="text" id="searchReport" class="form-control" placeholder="Tìm theo tiêu đề, nội dung hoặc người gửi">
                ...
                <div class="list">
                    <ul id="myUL" style="list-style-type: none;">
                        <li>
                            <img class="avatarRpt" src="${avatar}" onerror="this.src='report_image_userMale.png'" alt="">
                            <div class="noidung" onclick="ViewReport(this, '${md_thongbao_id}')">
                                <div class="fa fa-caret-left"></div>
            ...
            <script type="text/javascript">
                ...
                var ViewReport = function (elem, id) {
                    ...
                    if (styleA.length > 0) {
                        ...
                        $.ajax({
                            url: '../Controller/JQGridModify/JQGridMD_00_TBModify.ashx?oper=viewReport',
                            method: 'POST',
                            data: { id: id },
                            success: function (rs) { },
                            fail: function (rs) { },
                            complete: function () {
                                channelB.postMessage({ checkNewReport: true });
                            }
                        });
                    }
                };
                ...
                let loadReportFromServer = function (keyword) {
                    displayLoading(true);
                    $.ajax({
                        url: '../Controller/JQGridModify/JQGridMD_00_TBModify.ashx?oper=getJSON',
                        method: 'POST',
                        data: dataSeach,
                        success: function (rs) {
                            let maUser = '<%=ma_user%>';
                            ...
                            for (let i in rs) {
                                ...
                                row.viewState = jsonNN.viewTime ? "" : " (Chưa xem)";
                            ...
                            if (myUL.html().length <= 0) {
                                ...
                                let contentSystem = '';
                                contentSystem += '<span style="color:red">Không tìm thấy thông báo nào</span>, hãy kiểm tra <b>dấu</b> và <b>khoảng cách</b> của từ khóa. Để hiển thị lại toàn bộ thông báo, bạn cần <b>xóa từ khóa</b> và nhấn phím "Enter".';
                                contentSystem += '<br><br>Hoặc đơn giản là do chưa có ai gửi thông báo tới bạn, nếu vẫn còn vướng mắc xin vui lòng liên hệ với quản trị viên để biết thêm chi tiết.';
                        ...
                        complete: function () {
                            ...
                            $('a[file]').each(function () {
                                ...
                                $(this).click(function () {
                                    ...
                                    window.location.href = `${link}?id=${file}&cookieId=${id}`;
