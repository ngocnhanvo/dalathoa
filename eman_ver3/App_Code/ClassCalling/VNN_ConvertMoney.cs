...
{
    public static Dictionary<string, double> convert(double tien, string donvitien)
    {
        string kq = "";
		if(donvitien == "VND"){
            if (tien > 0) {
                tien = Math.Ceiling(tien);
        ...
        var kqDic = new Dictionary<string, double>();
        kqDic.Add(kq, tien);
        return kqDic;
	}
}